 ...
 * $Header$
 * $Revision: 208582 $
 * $Date: 2005-03-17 15:20:59 +0800 (Thu, 17 Mar 2005) $
 *
 * ====================================================================
 *
...
public class WebdavResource extends WebdavSession {
    ...
    public WebdavResource(HttpURL httpURL, String filePath, String proxyHost, int proxyPort,
          Credentials proxyCredentials, boolean followRedirects)
        throws HttpException, IOException {       
        setFollowRedirects(followRedirects);
        setProxy(proxyHost, proxyPort);
        setProxyCredentials(proxyCredentials);
        setHttpURL(httpURL, filePath);
    }
     ...
     */
    protected static int defaultAction = ALL; //BASIC;
    ...
    protected void setBasicProperties(int depth)
        ...
        properties.addElement(CREATORDISPLAYNAME);
     ...
     * @exception IOException
     */
    public boolean getMethod(OutputStream oStream) throws IOException {
     ...
     * @exception IOException
     */
    public boolean getMethod(OutputStream oStream, boolean closeStream)
        throws HttpException, IOException {

        setClient();
        GetMethod method = new GetMethod(URIUtil.encodePathQuery(httpURL.getPathQuery()));
...
/*            Enumeration responseProperties = method.getResponseProperties(href);
    ...
    public Enumeration propfindMethod(String path, Vector properties)
        ...
        if (properties != null)
            method = new PropFindMethod(URIUtil.encodePath(path),
                                                   DepthSupport.DEPTH_0,
                                                   properties.elements());
        else
            method = new PropFindMethod(URIUtil.encodePath(path),
        ...
        if (responses.hasMoreElements()) {
            ...
            while (responseProperties.hasMoreElements()) {
                Property property =
                    (Property) responseProperties.nextElement();
                String[] res = {property.getName(), property.getPropertyAsString()};
     ...
     * @exception IOException
     */
    public boolean userMethod(String username, String password, String firstname, String lastname, String email)
        throws HttpException, IOException {

       setClient();
        UserMethod method = new UserMethod(URIUtil.encodePath(httpURL.getPath()));
        method.setDebug(debug);
        method.setFollowRedirects(this.followRedirects);
    ...
    protected void processProperty(Property property) {
        ...
        else if (property.getLocalName().equals(CREATORDISPLAYNAME)) {
            creatorDisplayName = property.getPropertyAsString();
        }
        else if (property.getLocalName().equals(COMMENT)) {
            comment = property.getPropertyAsString();
        }
    }

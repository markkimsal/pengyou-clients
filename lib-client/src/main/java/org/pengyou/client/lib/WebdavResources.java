...
/**
 * This WebdavResources class represents a set of {@link DavResource WebDAV resources }.
 *
 * Two WebdavResource instances are considered equal, if there {@link DavResource#getName()
...
public class WebdavResources {
     ...
     */
    public WebdavResources(DavResource resource) {
     ...
     */
    public boolean isThereResource(DavResource resource) {
     ...
     */
    public DavResource[] listResources() {

        synchronized (hrefTable) {
            int num = hrefTable.size();
            DavResource WebdavResources[] = new DavResource[num];

            Enumeration resources = getResources();
            int i = 0;
            while (resources.hasMoreElements()) {
                WebdavResources[i++] =
                    (DavResource) resources.nextElement();
            }
     ...
     */
    public DavResource getResource(String resourceName) {
        return (DavResource) hrefTable.get(resourceName);
    }
     ...
     * @see #addResource(DavResource)
     */
    public void addResource(String resourceName, DavResource resource) {
     ...
     */
    public DavResource removeResource(String resourceName) {
        return (DavResource) hrefTable.remove(resourceName);
    }
